...
public:
  ...
  { return this->handle_close(ev_handler::all_events_mask); }
protected:
  int accept_i(inet_address *remote_addr)
  {
    ...
    do {
      new_handle = ::accept4(this->listen_fd_,
  ...
  int open_i(const inet_address &local_addr,
             ...
             const int backlog)
  {
    this->listen_fd_ = ::socket(AF_INET, SOCK_STREAM | SOCK_NONBLOCK | SOCK_CLOEXEC, 0);
  ...
  virtual int handle_input(const int )
  {
    do {
      ...
      if (sh->open(this) == -1)
        sh->close(ev_handler::accept_mask); 
    } while (true);
